...
class STRNN_final(nn.Module):
    ...
    # Normalize static frames only
        x_sm = normalize_data(x[:-1])  # [D, 3, H, W]

        # 1) Static + Motion features from STRNN_Static_Net
        out_sf, x_sf, out_mf, x_mf = self.feat_sm(x_sm, cb)

        # 2) Fuse static spatial + static temporal only
        # because cat_type=[1,1,0,0]
        x_fu = torch.cat([x_sf, x_mf], dim=1)  # [B*D, 512, 60, 80]

        # 3) Channel attention + Fusion refinement
        x_fu = self.att_channel(x_fu)
        x_fu = self.feat_fu(x_fu)

        # 4) Apply AA-LSTM
        B_D, C, H, W = x_fu.size()
        x_fu = x_fu.view(1, B_D, C, H, W)
        x_fu, x_state = self.att_lstm(x_fu, in_state)
        x_fu = x_fu.view(B_D, x_fu.size(2), H, W)

        # 5) Output 1×HxW map
        out = F.relu(self.conv_out(x_fu))  # [B*D, 1, 60, 80]
...
class STRNN_SRFu_woAALSTM(nn.Module):
    ...
    def forward(self, x, cb, in_state=None):
        ...
        # Only use static spatial + static temporal branches
        out_sf, x_sf, out_mf, x_mf = self.feat_sm(x_sm, cb)

# Expected input for SR-Fu
        x_fu = torch.cat([x_sf, x_mf], dim=1)   # [B*D, 512, 60, 80]
